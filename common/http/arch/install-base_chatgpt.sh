...
# - Disable encryption with --no-encryption
#
...
Options:
  --disk /dev/sdX|/dev/nvme0n1  Target disk (REQUIRED for --non-interactive).
  ...
  --no-encryption               Disable LUKS + LVM. Use plain ext4 on partition 2.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-encryption) ENCRYPTION=0; shift ;;
...
if (( NON_INTERACTIVE )); then
  [[ -n "$DISK" ]] || err "--disk is required in --non-interactive mode."
  : "${USERNAME:=vagrant}"
  : "${USERPASS:=vagrant}"
  : "${ROOTPASS:=vagrant}"
fi
...
if (( ! NON_INTERACTIVE )); then
  ...
  [[ -n "$DISK" ]] || read -r -p "Target disk (example: /dev/nvme0n1): " DISK
  ...
  if (( ENCRYPTION )); then
    info "Full-disk encryption is ENABLED by default. Use --no-encryption to disable."
  else
    warn "Encryption DISABLED by user flag --no-encryption."
  fi
...
# Per Arch guide: pacstrap /mnt <pkgs>; genfstab -U
pacstrap -K /mnt "${PKGS[@]}"
...
if (( NON_INTERACTIVE )); then
  ...
  : # already captured interactively
fi
...
# Secure sshd defaults: disable root login, disable password auth
